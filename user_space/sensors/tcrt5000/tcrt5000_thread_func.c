...
void* tcrt5000_thread_func(void* arg){
	...
	while(!stop_flag){
		...
		if(tcrt5000_read(&sensor) == 0){
			...
			// 循跡條件判斷
			switch(code){
				case 0: printf("000 -> 狀態: 出軌了 	     調整: 立即停車\n"); 		break;	
				case 2: printf("010 -> 狀態: 直線行駛 	     調整: 馬達保持轉速\n"); 		break;
				case 3: printf("011 -> 狀態: 車體微左偏      調整: 微右調(左邊馬達提速1)\n"); 	break;
				case 1: printf("001 -> 狀態: 左偏了 	     調整: 右調(左邊馬達提速2)\n"); 	break;
				case 6: printf("110 -> 狀態: 微右偏 	     調整: 微左調(右邊馬達提速1)\n"); 	break;
				case 4: printf("100 -> 狀態: 右偏了 	     調整: 左調(右邊馬達提速2)\n"); 	break;
