...
// 設定參數區
// ============================================================================
#define DERAIL_SET_COUNT 8       // 改為 10 次 
...
// 取得左右兩邊
int tcrt_get_left(void){
    tcrt5000_data data;
    if(tcrt5000_read(&data) == 0){
        return data.left;
    }
    return 0; // 讀取失敗當作沒偵測到
}

int tcrt_get_right(void){
    tcrt5000_data data;
    if(tcrt5000_read(&data) == 0){
        return data.right;
    }
...
// 主程式 (整合 UART 收發)
// ============================================================================
void uart_rx_handler(const char *data, size_t len){
    char c = data[0];
    printf("[UART] 收到字元: %c\n", c);


    // 輸送帶完成回報
    if(strncmp(data, "motor_off", 9) == 0){
        printf("[UART] 輸送帶完成回報\n");
        uart_send("l");  // 關左橘燈
        uart_send("o");  // 關右橘燈
    }
...
int main(){
    ...
    while(!stop_flag){
		...
		// 更新感測器數值
		sensor_left = tcrt_get_left();
		sensor_right = tcrt_get_right();

		// 檢查紅燈是否要關
		if(red_on && (sensor_left > 0 || sensor_right > 0)){
			uart_send("r");       // 關紅燈
			red_on = false;       // 標記紅燈已關.
        ...
        // 其他循跡邏輯可以在 handle_state() 處理
        usleep(50000); // 50ms 循環
    }
